...
export default {
  ...
  computed: {
    ...
    selectedText() {
      const selectrdNumber = this.shelfSelected ? this.shelfSelected.length : 0;
      return selectrdNumber <= 0
        ? this.$t("shelf.selectBook")
        : selectrdNumber === 1
        ? this.$t("shelf.haveSelectedBook").replace("$1", selectrdNumber)
        : this.$t("shelf.haveSelectedBooks").replace("$1", selectrdNumber);
    },
    popupCancelBtn() {
